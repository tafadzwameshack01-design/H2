...
def render_tab_picks(count: int, newly_graded: int) -> None:
    ...
    with st.spinner(""):
        st.markdown(
            '<div class="scan-line">⚡ ZEUS v4 SCANNING 75+ LEAGUES — MULTI-BET INTELLIGENCE ⚡</div>',
            unsafe_allow_html=True
        )
...
def main() -> None:
    ...
    count = 0
    if _AUTOREFRESH_OK:
        count = st_autorefresh(interval=60_000, key="zeus_v4_autorefresh")
